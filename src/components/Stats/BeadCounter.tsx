    ...
    <div className="flex flex-col h-full select-none">
      <div className="px-2 py-1.5 border-b bg-gray-50">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-semibold text-gray-600">{t("stats.title")}</h3>
        ...
        <p className="text-[10px] text-gray-400">
          {t("stats.summary", { colors: totalColors, beads: totalBeads })}
          {counts.length > 0 && (
            <button
              onClick={() => {
                const header = `${t("stats.code")},${t("stats.color")},HEX,${t("stats.count")}\n`;
      ...
      <div className="flex-1 overflow-y-auto">
        {counts.length === 0 ? (
          <p className="text-xs text-gray-400 p-3 text-center">{t("stats.empty")}</p>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="text-gray-500 border-b">
                <th className="px-2 py-1 text-left">{t("stats.color")}</th>
                <th className="px-2 py-1 text-left">{t("stats.code")}</th>
                <th className="px-2 py-1 text-right">{t("stats.count")}</th>
              </tr>
            </thead>
            <tbody>
              {counts.map((c) => (
                ...
                >
                  <td className="px-2 py-0.5">
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <span className="text-gray-600 truncate max-w-[60px]">{i18n.language === "zh-CN" ? c.name : c.code}</span>
                    </div>
                  </td>
